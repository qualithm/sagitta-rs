...
impl MetadataQuery {
  ...
  pub fn to_handle(&self) -> Bytes {
    match self {
      ...
      MetadataQuery::GetDbSchemas { .. } => Bytes::from(format!("{METADATA_HANDLE_PREFIX}schemas")),
      ...
      MetadataQuery::GetTableTypes => Bytes::from(format!("{METADATA_HANDLE_PREFIX}table_types")),
      ...
      MetadataQuery::GetSqlInfo { .. } => Bytes::from(format!("{METADATA_HANDLE_PREFIX}sql_info")),
  ...
  pub async fn get_db_schemas(
    ...
    if let Ok(datasets) = self.store.list(None).await {
      for dataset in &datasets {
        let (_, schema_name, _) = crate::catalog::StoreCatalog::path_to_catalog_schema_table(
          &dataset.path,
          &self.catalog_name,
          &self.default_schema,
        );
