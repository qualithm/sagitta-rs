...
impl SqlEngine {
  ...
  fn path_to_table_name(&self, path: &DataPath) -> String {
    let (_, schema, table) =
      StoreCatalog::path_to_catalog_schema_table(path, &self.catalog_name, &self.default_schema);
        ...
        match op {
          ...
          WriteOp::Update => {
            ...
            let modified_batches = modified_df
              ...
              .await
              .map_err(|e| SqlError::QueryExecution(e.to_string()))?;
            let affected_count: i64 = modified_batches.iter().map(|b| b.num_rows() as i64).sum();
          ...
          WriteOp::Delete => {
            ...
            let condition = Self::extract_filter_from_plan(input)
              ...
              .transpose()?
              .ok_or_else(|| {
                SqlError::Internal("could not extract WHERE clause from DELETE plan".to_string())
              })?;
            let qualified = self.qualified_table_sql(&path);

            let surviving_query = format!("SELECT * FROM {qualified} WHERE NOT ({condition})");
            let sdf = ctx
              .sql(&surviving_query)
              .await
              .map_err(|e| SqlError::QueryExecution(e.to_string()))?;
            let surviving_batches = sdf
              ...
              .await
              .map_err(|e| SqlError::QueryExecution(e.to_string()))?;
            let new_count: i64 = surviving_batches.iter().map(|b| b.num_rows() as i64).sum();
  ...
  async fn execute_ddl(&self, ddl: &DdlStatement) -> SqlResult<(DataPath, i64)> {
    match ddl {
      ...
      DdlStatement::CreateView(CreateView {
        ...
        // Register the view
        ctx
          .register_table(name.clone(), df.into_view())
          .map_err(|e| SqlError::Internal(e.to_string()))?;
  ...
  fn extract_filter_from_plan(plan: &LogicalPlan) -> Option<datafusion::logical_expr::Expr> {
    match plan {
      ...
      LogicalPlan::SubqueryAlias(alias) => Self::extract_filter_from_plan(alias.input.as_ref()),
      _ => None,
    }
  }
  ...
  fn alter_drop_column(
    ...
    let new_batches = self.drop_column_from_batches(current_batches, column_idx, &new_schema)?;
  ...
  async fn execute_upsert(&self, query: &str) -> SqlResult<(DataPath, i64)> {
    ...
    let values_subquery = format!("(SELECT * FROM (VALUES {values_section}) AS _nv({col_list}))");
  ...
  fn parse_insert_columns_values(&self, clause: &str) -> SqlResult<(Vec<String>, Vec<String>)> {
    ...
    let cols_end = clause
      .find(')')
      .ok_or_else(|| SqlError::SyntaxError("missing closing paren in column list".to_string()))?;
  ...
  fn substitute_parameters(query: &str, params: &[Arc<RecordBatch>]) -> SqlResult<String> {
    ...
    for col_idx in 0..batch.num_columns() {
      let scalar = ScalarValue::try_from_array(batch.column(col_idx), 0)
        .map_err(|e| SqlError::Internal(format!("failed to extract parameter {col_idx}: {e}")))?;
      let sql_expr = unparser
        .expr_to_sql(&DfExpr::Literal(scalar, None))
        .map_err(|e| SqlError::Internal(format!("failed to serialise parameter {col_idx}: {e}")))?;
  ...
  pub async fn begin_transaction(&self) -> SqlResult<Bytes> {
    self
      .begin_transaction_with_isolation(IsolationLevel::default())
      .await
  }
  ...
  pub fn end_savepoint(&self, savepoint_id: &Bytes, action: EndSavepoint) -> SqlResult<()> {
    ...
    match action {
      ...
      EndSavepoint::Rollback => {
        ...
        let discarded_count = transaction.pending_operations.len() - savepoint.operation_index;
        transaction
          .pending_operations
...
mod tests {
  ...
  async fn test_execute_insert_multiple_rows() {
    ...
    let cmd = CommandStatementUpdate {
      query: "INSERT INTO test.table (id, value) VALUES (4, 40), (5, 50), (6, 60)".to_string(),
      transaction_id: None,
    };
  ...
  async fn test_information_schema_columns() {
    ...
    let cmd = CommandStatementQuery {
      query: "SELECT * FROM information_schema.columns WHERE table_name = 'table'".to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
    assert!(!result.schema.fields().is_empty());
  ...
  async fn test_union_all() {
    ...
    let cmd = CommandStatementQuery {
      query: "SELECT id FROM test.\"table\" UNION ALL SELECT id FROM test.\"table\"".to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
    assert!(!result.schema.fields().is_empty());
  ...
  async fn test_intersect() {
    ...
    let cmd = CommandStatementQuery {
      query: "SELECT id FROM test.\"table\" INTERSECT SELECT id FROM test.\"table\"".to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
    assert!(!result.schema.fields().is_empty());
  ...
  async fn test_except() {
    ...
    let cmd = CommandStatementQuery {
      query: "SELECT id FROM test.\"table\" EXCEPT SELECT id FROM test.\"table\" WHERE id = 1"
        .to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
  ...
  async fn test_case_expression() {
    ...
    let cmd = CommandStatementQuery {
      query:
        "SELECT id, CASE WHEN value > 15 THEN 'high' ELSE 'low' END as category FROM test.\"table\""
          .to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
  ...
  async fn test_window_function_row_number() {
    ...
    let cmd = CommandStatementQuery {
      query: "SELECT id, ROW_NUMBER() OVER (ORDER BY id) as rn FROM test.\"table\"".to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_query(&cmd).await.unwrap();
    assert_eq!(result.schema.fields().len(), 2);
  ...
  async fn test_insert_select_basic() {
    ...
    let insert_query = "INSERT INTO dest_table SELECT id, value FROM test.integers WHERE id < 10";
    let cmd = CommandStatementUpdate {
  ...
  async fn test_truncate_table_basic() {
    ...
    let create_query = "CREATE TABLE truncate_test AS SELECT * FROM test.integers WHERE id < 50";
    let cmd = CommandStatementUpdate {
      query: create_query.to_string(),
      transaction_id: None,
    };
  ...
  async fn test_truncate_without_table_keyword() {
    ...
    // Create a table
    let create_query = "CREATE TABLE truncate_test2 AS SELECT * FROM test.integers WHERE id < 20";
    let cmd = CommandStatementUpdate {
      query: create_query.to_string(),
      transaction_id: None,
    };
  ...
  async fn test_alter_table_drop_column() {
    ...
    let create_query = "CREATE TABLE drop_test AS SELECT id, value FROM test.integers WHERE id < 5";
    let cmd = CommandStatementUpdate {
      query: create_query.to_string(),
      transaction_id: None,
    };
  ...
  async fn test_upsert_do_nothing() {
    ...
    let cmd = CommandStatementUpdate {
      query: "INSERT INTO upsert_test (id, value) VALUES (1, 999) ON CONFLICT (id) DO NOTHING"
        .to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_update(&cmd).await;
  ...
  async fn test_upsert_table_not_found() {
    ...
    let cmd = CommandStatementUpdate {
      query: "INSERT INTO nonexistent (id) VALUES (1) ON CONFLICT (id) DO NOTHING".to_string(),
      transaction_id: None,
    };
    let result = engine.execute_statement_update(&cmd).await;
