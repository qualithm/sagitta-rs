...
use arrow_flight::{
  ...
  sql::{
    ...
    ActionCreatePreparedStatementRequest, ActionEndSavepointRequest, ActionEndTransactionRequest,
    Any, Command, CommandPreparedStatementQuery, DoPutPreparedStatementResult, DoPutUpdateResult,
    EndTransaction, TicketStatementQuery,
  },
};
...
impl SagittaService {
  ...
  #[allow(dead_code)]
  #[allow(clippy::result_large_err)]
  fn authenticate_request_optional<T>(&self, request: &Request<T>) -> Result<Option<User>, Status> {
    ...
    S: Stream<Item = Result<FlightData, Status>> + Unpin + Send + 'static,
  {
    ...
    match mode {
      "echo" => {
        ...
        while let Some(result) = decoder
          .as_mut()
          .try_next()
          .await
          .map_err(|e| Status::invalid_argument(format!("failed to decode flight data: {e}")))?
        {
          batches.push(result);
        }
      ...
      "query" => {
        ...
        let sql = std::str::from_utf8(&first.app_metadata)
          .map_err(|_| {
            Status::invalid_argument("query exchange: app_metadata must be valid UTF-8 SQL")
          })?
          .trim()
          .to_string();
    ...
    S: Stream<Item = Result<FlightData, Status>> + Unpin + Send + 'static,
  {
    ...
    match command {
      Command::CommandPreparedStatementQuery(prepared_cmd) => {
        ...
        self
          .do_put_prepared_statement_parameters(prepared_cmd, first, stream)
          .await
      }
    ...
    S: Stream<Item = Result<FlightData, Status>> + Unpin + Send + 'static,
  {
    ...
    if has_data {
      ...
      while let Some(result) = decoder
        ...
        .try_next()
        .await
        .map_err(|e| Status::invalid_argument(format!("failed to decode parameter data: {e}")))?
      {
  ...
  async fn do_get_statement_query(
    ...
    if MetadataEngine::is_metadata_handle(handle) {
      ...
      let batch_stream = futures::stream::iter(batches.into_iter().map(|b| Ok(b.as_ref().clone())));
...
impl FlightServiceTrait for SagittaService {
  ...
  async fn get_schema(
    ...
    let result: SchemaResult = schema_ipc
      .try_into()
    ...
    match action.r#type.as_str() {
      ...
      "CreatePreparedStatement" => {
        ...
        let request: ActionCreatePreparedStatementRequest = any
          ...
          .ok_or_else(|| {
            Status::invalid_argument("action body is not CreatePreparedStatement request")
          })?;
      ...
      "ClosePreparedStatement" => {
        ...
        let request: ActionClosePreparedStatementRequest = any
          ...
          .ok_or_else(|| {
            Status::invalid_argument("action body is not ClosePreparedStatement request")
          })?;
      ...
      "BeginTransaction" => {
        ...
        let _request: ActionBeginTransactionRequest = any
          .unpack()
          .map_err(|e| {
            Status::invalid_argument(format!("failed to decode BeginTransaction request: {e}"))
          })?
          .ok_or_else(|| Status::invalid_argument("action body is not BeginTransaction request"))?;
      ...
      "EndTransaction" => {
        ...
        let request: ActionEndTransactionRequest = any
          .unpack()
          .map_err(|e| {
            Status::invalid_argument(format!("failed to decode EndTransaction request: {e}"))
          })?
          .ok_or_else(|| Status::invalid_argument("action body is not EndTransaction request"))?;
      ...
      "BeginSavepoint" => {
        ...
        let request: ActionBeginSavepointRequest = any
          .unpack()
          .map_err(|e| {
            Status::invalid_argument(format!("failed to decode BeginSavepoint request: {e}"))
          })?
          .ok_or_else(|| Status::invalid_argument("action body is not BeginSavepoint request"))?;
      ...
      "EndSavepoint" => {
        ...
        let request: ActionEndSavepointRequest = any
          .unpack()
          .map_err(|e| {
            Status::invalid_argument(format!("failed to decode EndSavepoint request: {e}"))
          })?
          .ok_or_else(|| Status::invalid_argument("action body is not EndSavepoint request"))?;
      ...
      "CancelFlightInfo" => {
        ...
        // Decode the request
        let request = CancelFlightInfoRequest::decode(action.body.as_ref()).map_err(|e| {
          Status::invalid_argument(format!("failed to decode CancelFlightInfoRequest: {e}"))
        })?;
  ...
  async fn list_actions(
    ...
    let mut actions = vec![
      ...
      ActionType {
        ...
        description: "Create a prepared statement. Body: ActionCreatePreparedStatementRequest."
          .to_string(),
      },
      ActionType {
        r#type: "ClosePreparedStatement".to_string(),
        description: "Close a prepared statement. Body: ActionClosePreparedStatementRequest."
          .to_string(),
      },
      ActionType {
        r#type: "BeginTransaction".to_string(),
        description: "Begin a new transaction. Body: ActionBeginTransactionRequest.".to_string(),
      },
      ActionType {
        r#type: "EndTransaction".to_string(),
        description: "End a transaction (commit or rollback). Body: ActionEndTransactionRequest."
          .to_string(),
      },
      ActionType {
        r#type: "BeginSavepoint".to_string(),
        description: "Create a savepoint within a transaction. Body: ActionBeginSavepointRequest."
          .to_string(),
      },
      ActionType {
        r#type: "EndSavepoint".to_string(),
        description: "End a savepoint (release or rollback). Body: ActionEndSavepointRequest."
          .to_string(),
      },
      ActionType {
...
mod tests {
  ...
  async fn test_get_flight_info_existing_path() {
    ...
    let descriptor = FlightDescriptor::new_path(vec!["test".to_string(), "integers".to_string()]);
  ...
  async fn test_do_exchange_echo_simple() {
    ...
    let descriptor = FlightDescriptor::new_path(vec!["exchange".to_string(), "echo".to_string()]);
  ...
  async fn test_do_exchange_store_missing_data_path_error() {
    ...
    let descriptor = FlightDescriptor::new_path(vec!["exchange".to_string(), "store".to_string()]);
  ...
  async fn test_do_exchange_query_mode() {
    ...
    let descriptor = FlightDescriptor::new_path(vec!["exchange".to_string(), "query".to_string()]);

    let empty_stream = futures::stream::empty::<Result<arrow_array::RecordBatch, FlightError>>();
